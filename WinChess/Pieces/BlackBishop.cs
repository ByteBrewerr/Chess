...
namespace WinChess.Pieces
{
    ...
    {
        public BlackBishop(int _row, int _col)
        {
            this.row = _row;
            this.col = _col;
            this.Image = new Bitmap(Properties.Resources.blackBishop);
            this.IsBlack = true;
        }
        ...
        public override bool IsCanMove(ChessBoard Board, int toR, int toC, bool onlyCheck)
        {
            ...
            if (Board[toR, toC] != null && Board[toR, toC].IsBlack)  //Если фигура черная в конце, то ходить нельзя
                return false;
