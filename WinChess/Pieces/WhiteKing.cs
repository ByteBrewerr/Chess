...
namespace WinChess.Pieces
{
    ...
    {
        public WhiteKing(int _row, int _col)
        {
            this.row = _row;
            this.col = _col;
            this.Image = new Bitmap(Properties.Resources.whiteKing);
            this.IsWhite = true;
        }
        ...
        public override bool IsCanMove(ChessBoard Board, int toR, int toC, bool onlyCheck)
        {
            ...
            {
                if (!hasMoved && Board[0, 7] != null && !Board[0, 7].hasMoved)
                {
            ...
            {
                if (!hasMoved && Board[0, 0] != null && !Board[0, 0].hasMoved)
                {
        ...
        public override bool IsInCheck(ChessBoard Board)
        {
            for (int r = 0; r < 8; r++)
            {
                for (int c = 0; c < 8; c++)
                {
                    if (Board[r, c] != null && Board[r, c].IsBlack)
                    {
